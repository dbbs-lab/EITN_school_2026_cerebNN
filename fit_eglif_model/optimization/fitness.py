...
from .features import inv_first_isi, inv_second_isi
...
def time_to_first_spike_loss(targ, nest, mode="max"):
    try:
        tt = targ['time_to_first_spike'].values.astype(float)
        tn = nest['time_to_first_spike'].values.astype(float)
        diff = np.abs(tt - tn)
        
        if np.any(np.isnan(diff)):
            return 1.0
        
        losses = (np.clip(diff / (1 + diff), 0.0, 1.0))
        if mode == "max":
